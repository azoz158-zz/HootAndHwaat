...
@inject("store")
@observer
export default class FisherTripsOrders extends Component {
  constructor(props) {
    super(props);
    this.state = {
        page: 0,
...
componentDidMount(){
    axios.get('http://hootandhawat-001-site1.ftempurl.com/api/get/orders/all/trips/customer_id='+ this.props.store.userId+'&page=1', {
    ...
    return (
      <View style={{ flex: 1 }}>
        <Header style={{ backgroundColor: '#6fc1bb', alignContent: 'center', alignItems: 'center', height: 40, paddingTop: 0 }}>
          ...
          <Body>
            <Title style={{ fontSize: responsiveFontSize(2.5) }}>{this.props.store.strings.trips_orders}</Title>
          </Body>
          <Right>
        ...
        <FlatList
                        ...
                        renderItem={({ item, index }) => (<CustomerTripOrderUnit data={item} ind={index}

                        >

                        </CustomerTripOrderUnit>)}
                    />
