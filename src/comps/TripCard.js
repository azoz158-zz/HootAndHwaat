    ...
    renderTickets = () => {
        ...
        return (
            <Dialog.Container visible={this.state.numberVisible}>
                <Dialog.Title>عدد التذاكر المطلوبة</Dialog.Title>
    ...
    renderBtn = () => {
        if (this.props.store.userType == 'user' || this.props.store.userType == null) {
            if (this.state.buying == false) {
                return (<Button block warning style={{ width: 150, height: 40, alignItems: 'center', alignContent: 'center', justifyContent: 'center' }} onPress={() => this.showTickets()}>
