...
@inject("store")
export default class CustomerTripOrderUnit extends Component {
    ...
    accept = () => {
        ...
        axios.post('http://hootandhawat-001-site1.ftempurl.com/api/trip/complete', {
    ...
    refuse = () => {
        ...
        axios.post('http://hootandhawat-001-site1.ftempurl.com/api/trip/refused', {
            order_id: this.props.data.order_id
        }, {
                headers: {
                    AuthKey: this.props.store.authKey,
            ...
            <View style={{ borderColor: 'grey', borderWidth: 1, paddingBottom: 5, borderRadius: 10, marginBottom: 5 }}>
                <View style={{ width: responsiveWidth(100), flexDirection: 'row', height: responsiveHeight(25), justifyContent: 'space-between', padding: 10 }}>
                    <View style={{ width: "48%", justifyContent: 'space-between' }}>
                        ...
                        <Text style={{ fontFamily: Platform.OS === 'ios' ? 'CoconNextArabic-Light' : 'Cocon® Next Arabic-Light', fontSize: responsiveFontSize(2), width: '100%', textAlign: 'right' }}>{this.props.store.strings.cust_name}: {this.props.data.username}</Text>
                        ...
                        <Text style={{ fontFamily: Platform.OS === 'ios' ? 'CoconNextArabic-Light' : 'Cocon® Next Arabic-Light', fontSize: responsiveFontSize(2), width: '100%', textAlign: 'right' }}>{this.props.store.strings.duration}: {this.props.data.trip_time} {this.props.store.strings.hours}</Text>
